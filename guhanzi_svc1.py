...
for file in sorted(os.listdir("GuHanZi_dataset_bigsplit/trainset")):
  if(not "jpg" in file or "jia" in file):
    continue
  file = "GuHanZi_dataset_bigsplit/trainset/" + file
  img = cv2.imread(file, 0)
  img = np.float32(img)/255
  if(model_type == "svc"):
    img = overlap_patches(img, 14, 8)
...
optimizer = torch.optim.Adam(params, lr = 0.00005, weight_decay = 0.02)
...
for epoch in range(num_epochs):
  ...
  lr_scheduler.step()
train_acc = train_acc*100/len(train_losses)/batches
